...
public class ProductoRepo2Impl implements ProductoRepo2 {
    ...
    public Producto2 buscarProductoNombre(String nombre) {
        List<Producto2> productoEncontrado = entityManager.createNativeQuery("SELECT * FROM producto2 p where p.nombre like ? ", Producto2.class).setParameter(1, nombre).getResultList();
